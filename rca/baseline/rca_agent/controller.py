...
from rca.MCPToolWrapper import MCPToolWrapper,process_with_mcp
...
async def control_loop(objective:str, plan:str, ap, bp, logger, max_step = 15, max_turn = 3) -> str:
    ...
    for step in range(max_step):
        ...
        attempt_actor = []
        try:
            # response_raw =  process_with_mcp(prompt + note)
            ...
            if completed == "True":
                ...
                # answer = process_with_mcp(prompt)
            ...
            result , new_history = await execute_act(instruction, bp.schema, history, attempt_actor, logger)
            if not status:
            ...
            history = new_history
            trajectory.append({'code': f"#", 'result': f"Out[{step+1}]:\n```\n{result}```"})
            logger.info('-'*80 + '\n' + f"Step[{step+1}]\n### Observation:\n{result}" + '\n' + '-'*80)
            prompt.append({'role': 'assistant', 'content': response_raw})
    ...
    #     messages=prompt,
    # )
    answer = await  process_with_mcp(prompt)
    logger.debug(f"Raw Final Answer:\n{answer}")
    prompt.append({'role': 'assistant', 'content': answer})
    if "```json" in answer:
        answer = re.search(r"```json\n(.*)\n```", answer, re.S).group(1).strip()
