...
async def execute_act(instruction:str, background:str, history, attempt, logger) -> str:
    ...
    #             # )
    #             response = await process_with_mcp(prompt+note)
    #         else:
    #             # response = get_chat_completion(
    #             #     messages=prompt,
    #             # )
    #             response = await process_with_mcp(prompt)
    #
    response = await process_with_mcp(prompt)
...
#     prompt = history.copy()
#     note = [{'role': 'user',
#              'content': f"Continue your code writing process following the rules:\n\n{rule}\n\nResponse format:\n\n{format}"}]
#     tokenizer = tiktoken.encoding_for_model("gpt-4")
#     for i in range(2):
#         try:
#             if not retry_flag:
