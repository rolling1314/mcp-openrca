...
class RCA_Agent:
    ...
    async def run(self, instruction, logger, max_step=25, max_turn=5):
            
        logger.info(f"Objective: {instruction}")
        prediction, trajectory, prompt = await control_loop(instruction, "", self.ap, self.bp, logger=logger, max_step=max_step, max_turn=max_turn)
