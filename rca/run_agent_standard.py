...
async def main(args, uid, dataset):
    ...
    for idx, row in instruct_data.iterrows():
        ...
        for i in range(args.sample_num):
            ...
            try:
                with timeout_context(args.timeout):
                    agent = RCA_Agent(ap, bp)
                    prediction, trajectory, prompt =await agent.run(instruction,
                                                               logger,
...
if __name__ == "__main__":
    ...
    if args.auto:
        ...
        asyncio.run(main(args, uid, dataset))